# inbalance: cluster size is not an uniform distribution
library(splatter)
library(rhdf5)


setwd("G:/PycharmProjects/MSVGAE/R")
...
group.probs=list(
  ...
  c(0.01, 0.01, 0.04, 0.04, 0.2, 0.2, 0.2, 0.2, 0.01, 0.01, 0.04 , 0.04),
...
for(i in 1:length(group.probs)){
  for(j in 1:length(dropouts)){
    print(paste0("Simulating ",length(group.probs), " clusters and ", dropouts[j], "  dropout level  " ))
